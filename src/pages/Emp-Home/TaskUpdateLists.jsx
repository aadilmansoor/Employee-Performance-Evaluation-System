...
const TaskUpdateLists = () => {
    ...
    const fetchTaskUpdate = async () => {
      try {
        const response = await axios.get(
          "http://127.0.0.1:8001/empapi/taskupdateschart/",
          {
            headers: {
  ...
  return (
    <div className="mt-8 h-96 overflow-y-auto relative">
      <hr />
  <h1 className="text-2xl font-semibold mb-4 absolute top-0 left-0 w-full bg-white py-2 px-4 z-10">Updated Tasks</h1>
  {taskUpdate.length > 0 ? (
          ...
          <tbody className="divide-y divide-gray-200">
            ...
            {taskUpdate.map((taskupdate, index) => (
              <tr key={index}>
                ...
                <td className="py-3 px-4 border whitespace-nowrap">{taskupdate.updated_by}</td>
                <td className="py-3 px-4 border whitespace-nowrap">{new Date(taskupdate.date_updated).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })}</td>
                <td className="py-3 px-4 border whitespace-nowrap">{taskupdate.name}</td>
                <td className="py-3 px-4 border whitespace-nowrap">{taskupdate.description}</td>
                <td className="py-3 px-4 border whitespace-nowrap">{taskupdate.performance_level}</td>
              </tr>
            ))}
          </tbody>
        </table>
